...
{ 
    ...
    *mailHdr = mail->mailHdr;
    ...
    delete mail;                        // we've copied out the stuff we
                                        // need, we can now discard the 
                                        // message
}

//----------------------------------------------------------------------
// PostOfficeInput::PostOfficeInput
//      Initialize the post office input queues as a collection of 
//      mailboxes. Also initialize the network device, to allow post 
//      offices on different machines to deliver messages to one another.
//
...
// CHANGE:
// added PostalDelivery_st function for Fork call in PostOfficeInput
// nnoc, 10/1/2001
void
PostOfficeInput::PostalDelivery_st(PostOfficeInput *poi)
{
    poi->PostalDelivery();
}
...
PostOfficeInput::PostOfficeInput(int nBoxes)
{
    ...
    // CHANGE:
    // pass PostalDelivery_st not PostalDelivery, nnoc, 10/1/2001
    t->Fork((VoidFunctionPtr) &PostalDelivery_st, this);
}
