...
//	    exception handler, and when it returns, we return to Run(), 
//	    which will re-invoke us in a loop.  This allows us to
...
//	    This routine is re-entrant, in that it can be called multiple
//	    times concurrently -- one for each thread executing user code.
//	    We get re-entrancy by never caching any data -- we always 
//	    re-start the simulation from scratch each time we are called 
//	    (or after trapping back to the Nachos kernel on an exception 
//	    or interrupt), and we always store all data back to the machine 
//	    registers and memory before leaving.  This allows the Nachos 
//	    kernel to control our behavior by controlling the contents of 
//	    memory, the translation table, and the register set.
//----------------------------------------------------------------------
