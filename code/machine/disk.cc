...
Disk::Disk(CallBackObj *toCall)
{
    ...
    if (fileno >= 0) {                  // file exists, check magic number 
        ...
        WriteFile(fileno, (char *) &magicNum, MagicSize); 
                                        // write magic number
...
//      correct track on the disk.  Since when we finish seeking, we are 
//      likely to be in the middle of a sector that is rotating past the 
//      head, we also return how long until the head is at the next 
//      sector boundary.
//      
...
//      the disk head will be after the seek (if any).  We then figure out
...
//      allows read requests to the current track to be satisfied more 
//      quickly. The contents of the track buffer are discarded after 
//      every seek to a new track.
//----------------------------------------------------------------------
