...
//      to signify that we should idle the CPU until the next I/O 
//      interrupt occurs (the only thing that could cause a thread to 
//      become ready to run).
//
//      NOTE: we assume interrupts are already disabled, because it
//      is called from the synchronization routines which must
//      disable interrupts for atomicity.   We need interrupts off 
//      so that there can't be a time slice between pulling the first 
//      thread off the ready list, and switching to it.
//----------------------------------------------------------------------
void
...
//      Dummy functions because C++ does not (easily) allow pointers to 
//      member functions.  So we create a dummy C function
...
Thread::StackAllocate (VoidFunctionPtr func, void *arg)
{
    ...
    *stack = STACK_FENCEPOST;
    ...
    stackTop = stack + StackSize - 16;  // RS6000 requires 64-byte frame 
                                        // marker
    *stack = STACK_FENCEPOST;
#endif 
...
Thread::copyFiles(Thread * t)
{
       ...
       {
               files[i] = t->files[i];
               kernel->FileTable->upCount(files[i]);
       }
...
{
       fileCount--;
       for (int i = id; i < fileCount; i++)
               files[i] = files[i+1];
       files[fileCount] = -1;
}

bool
Thread::findFile(int id)
{
       int i;
       for(i=0;i<fileCount;i++)
       {
               if(files[i]==id)return true;
       }
